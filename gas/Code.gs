 ...
 *  Backend API · Deploy as Web App
 *  Execute as: Me | Who has access: Anyone
 *  ─────────────────────────────────────────────────────────
 *  การ Login ครั้งแรก: ใช้ employee_id เป็น PIN แรก
 *  แล้วระบบบังคับเปลี่ยน PIN 6 หลักใหม่ทันที
 * ============================================================
 */
...
function ok(data={})  { return res(JSON.stringify({success:true,...data})); }
function fail(msg,c=400){ return res(JSON.stringify({success:false,error:msg,code:c})); }
...
function getSettings(category) {
  ...
  const filtered = category ? rows.filter(r => r.category===category && r.active==='1') : rows.filter(r=>r.active==='1');
  return filtered.sort((a,b) => Number(a.sort_order) - Number(b.sort_order));
}

function getDepartments() { return getRows(SHEET.DEPARTMENTS, COL.DEPARTMENTS).map(r => r.name); }
...
function handleUpdateUser(p) {
  if (!p.id) return fail('ไม่พบ ID');
  const updates = {};
  ['first_name','last_name','email','phone_ext','role','status','avatar_url'].forEach(f => { if(p[f]!==undefined) updates[f]=p[f]; });
...
function handleGetStats() {
  ...
  records.forEach(r => {
    ...
    byDay[r.created_at.substring(0,10)]=(byDay[r.created_at.substring(0,10)]||0)+1;
  });
  const userMap={};
  users.forEach(u => userMap[u.employee_id]=`${u.first_name} ${u.last_name}`);
  const kpi = Object.entries(byAgent).map(([e,c])=>({employee_id:e,name:userMap[e]||e,count:c})).sort((a,b)=>b.count-a.count);
...
function handleInit() {
  ...
  if (sets.getLastRow()<=1) {
    [['os','win10','Windows 10','1'],['os','win11','Windows 11','2'],
     ['os','ubuntu16','Ubuntu 16','3'],['os','ubuntu20','Ubuntu 20.x','4'],
     ['status','pass','Pass','1'],['status','doi','ด๋อย','2'],['status','reject_user','Reject by user','3']
  ...
  if (usrs.getLastRow()<=1) {
    // ── PIN แรก = employee_id ──────────────────────────────────
    usrs.appendRow([generateId(),'admin001','System','Admin','admin@company.com','123456','admin','','','','active','1','',now()]);
  }

  return ok({ message: 'Init complete! — Login: employee_id=admin001, PIN=admin001 (รหัสพนักงาน)' });
}
